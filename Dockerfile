...
# Do setup steps to make the openwrt build agent with all required dependencies
RUN apt-get update
...
# Add and swap to a non-root user because we can't build as root
ARG user=cibuild
ARG group=builders
ARG uid=1000
ARG gid=1000
...
# Set up git so we can properly check out repos
